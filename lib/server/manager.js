...
------------------------------------------------------------------------------*/
Manager.prototype.attachSession = function(response, domain) {
    var session = new Session(Uuid());

    session.attachClient(response, this._DOMAIN_POOL[0] + domain);
...
/*------------------------------------------------------------------------------
  ...
  connection limits.
------------------------------------------------------------------------------*/
Manager.prototype.assignPollDomain = function(sessionID, domain) {
	...
	if(clients) {
		...
		for(itemDomain in this._DOMAIN_POOL) {
			item = this._DOMAIN_POOL[itemDomain] + domain;
		...
		if(!pollDomain) {
			pollDomain = this._DOMAIN_POOL[0] + domain;
		}
	}

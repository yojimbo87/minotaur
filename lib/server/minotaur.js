				...
				if(client.isPollReady) {
					if(client.messageBuffer.length > 0) {
						...
						if(client.response) {
							...
							self._sendResponse(client.response, client.messageBuffer);
						...
						// second)
						if(client.response && (client.ticks > 18)) {
							client.isPollReady = false;
							self._sendResponse(client.response, [{cmd: "poll"}]);
						}
					}
				}
...
Minotaur.prototype._message = function(request, response, queryString) {	
    ...
    if(sessionID && queryString) {
		...
		response.write(
			"_jsnpsnd(" +
			JSON.stringify({messages: [{cmd: "mess_ok"}]}) +
			");", 
			"utf8"
		);
		response.end();*/
		...
		response.write(
			"_jsnpsnd(" +
			JSON.stringify({messages: [{cmd: "mess_err"}]}) +
			");",
			"utf8"
		);
		response.end();*/
...
------------------------------------------------------------------------------*/
Minotaur.prototype._sendResponse = function(response, content, jsonCallbackName) {
	var callbackName = "_jsnpcb";
	
	if(jsonCallbackName) {
		callbackName = jsonCallbackName;
	}

	response.writeHead(200, {"Content-Type": "application/javascript"});
	response.write(
		callbackName + "(" + JSON.stringify({messages: content}) + ");", 
		"utf8"
	);
	response.end();
};
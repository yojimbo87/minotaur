...
Minotaur.prototype.init = function() {
	...
	this._httpServer.addListener("request", function(request, response) {
		...
		switch(path) {
			case "/connect":
				dotIndex = request.headers.host.indexOf(".");
				domain = request.headers.host.substring(dotIndex);

				self._connect(request, response, domain);
				break;
			case "/poll":
				self._poll(
					request, 
...
------------------------------------------------------------------------------*/
Minotaur.prototype._connect = function(request, response, domain) {	
	var self = this,
	...
	if(sessionID) { // session ID (from cookie) exist
		...
		if(session) { // session exist
			session.attachClient(
				response, 
				manager.assignPollDomain(sessionID, domain)
			);
		} else { // session doesn't exist
			session = manager.attachSession(response, domain);
